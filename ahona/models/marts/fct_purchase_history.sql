...
(
    ...
    FROM {{ref('stg_purchase_history.sql')}}
...
SELECT
   ...
   ROUND(fct.mtr_quantity * dprod.mtr_unit_price(1-fct.mtr_discount),2) AS mtr_total_amount_gross

FROM stg_fct_purchase_history AS fct
LEFT JOIN {{ref('dim_customers.sql')}} AS dcust ON 
        fct.nk_customer_id =  dcust.nk_customer_id 

LEFT JOIN {{ref('dim_channels.sql')}} AS dchan ON
        fct.nk_channel_id = dchan.nk_channel_id
LEFT JOIN {{ref('dim_products.sql')}} AS dprod ON
        fct.nk_product_sku = dprod.nk_product_sku